...
import type { DanmuContent, Danmu } from '@/lib/contract/types/danmu'

let danmu = ref<DanmuContent>({
  txt: '',
  t: '',
  fs: '',
  fc: '',
...
<template>
  ...
  <div>
    ...
    <div v-for="(a, index) in danmuList" :key="a.characterId + a.noteId">
      {{ index }} ---{{ a }} <button @click="DanmuClient.delDanmu(a.noteId)">删除</button>
    </div>
  </div>
</template>
